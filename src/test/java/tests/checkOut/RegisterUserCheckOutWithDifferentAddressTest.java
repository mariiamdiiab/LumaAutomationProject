...
@Epic("Checkout")
@Feature("Checkout with Different Address")
public class RegisterUserCheckOutWithDifferentAddressTest extends TestBase {
    ...
    public Object[][] provideNewAddressData() throws IOException {
    ...
    @Test(priority = 2, dependsOnMethods = "verifyUserCanAddProductToCart",
            dataProvider = "newAddressData", groups = "ValidTests")
    @Severity(SeverityLevel.CRITICAL)
    @Story("Checkout Process")
    @Description("Validate checkout process using new address")
    public void verifyCheckOutWithNewAddressSuccessfully(
            String tcId, String description, String street, String city,
            String state, String zip, String country, String phone) {

        Allure.description(description);
        Allure.parameter("Test Case ID", tcId);
        Allure.parameter("Street", street);
        Allure.parameter("City", city);
        Allure.parameter("State", state);
        Allure.parameter("Zip Code", zip);
        ...
        Allure.step("2. Complete checkout with new address", () ->
                checkOutPage.checkOutWithNewAddress(street, city, state, zip, country, phone));
        ...
        Allure.step("4. Retrieve and save order ID", () -> {
            ...
            Allure.addAttachment("Order ID", "text/plain", orderId);
            System.out.println("The order id is " + orderId);
        });

        Allure.addAttachment("Test Case Result",
                "Test case " + tcId + " passed successfully.");
    }
}